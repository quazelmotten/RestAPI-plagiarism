...
class AnalysisService:
    ...
    def analyze_pair_full(
        ...
        def do_analyze():
            result = self.analyzer.analyze(file1_path, file2_path, language)
            return {
                "similarity_ratio": result.similarity_ratio,
                "matches": [
                    {"file1": m.file1, "file2": m.file2, "kgram_count": m.kgram_count}
        ...
        try:
            ...
            future.cancel()
            raise TimeoutError(f"Full analysis timed out after {timeout}s") from None
        except Exception:
            raise

    def generate_fingerprints(self, file_path: str, language: str, timeout: int = 600) -> dict:
        """
        ...
        try:
            ...
            future.cancel()
            raise TimeoutError(f"Fingerprint generation timed out after {timeout}s") from None
        except Exception:
            raise
