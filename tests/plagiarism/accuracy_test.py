...
from py_clone_generator import generate_dataset, CloneGenerator
...
def run_accuracy_test(
    ...
    print(f"=== Plagiarism Detection Accuracy Test ===")
...
def print_results(results: Dict[str, Any]):
    ...
    for type_name, data in sorted(results["by_type"].items()):
        ...
        print(f"    Medium (50-79%): {data['medium']} ({data['medium']/data['total']*100:.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Accuracy test for plagiarism detection algorithm"
    )
    ...
    parser.add_argument("--n-clones", type=int, default=1,
                        help="Number of clones per file per type (default: 1)")
