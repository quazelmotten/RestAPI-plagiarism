...
def compute_detailed_match_statistics(
    matches: list[dict], file_sizes: dict[str, int]
) -> dict[str, Any]:
...
def modify_analyzer_parameters(params: dict[str, Any]) -> bool:
    ...
    try:
        ...
        if "min_depth" in params:
            ...
            if idx < len(lines):
                lines[idx] = (
                    f"def extract_ast_hashes(file_path, lang_code, min_depth={params['min_depth']}, tree=None):\n"
                )
                modified.append(idx)
            ...
            for call_idx in call_line_indices:
                if call_idx < len(lines) and "min_depth=3" in lines[call_idx]:
                    lines[call_idx] = lines[call_idx].replace(
                        "min_depth=3", f"min_depth={params['min_depth']}"
                    )
        ...
        if "minimum_occurrences" in params:
            ...
            if idx < len(lines):
                lines[idx] = lines[idx].replace(
                    "minimum_occurrences=1", f"minimum_occurrences={params['minimum_occurrences']}"
                )
                modified.append(idx)
...
def run_accuracy_test_with_files(
    dataset_files: list[Path], cloned_files: list[tuple[Path, Path, int]], threshold: float = 0.30
) -> dict[str, Any]:
    ...
    results["summary"] = {
        ...
        "large_fragments_rate": (total_large_fragments / total_fragments * 100)
...
def calculate_smart_score(
    results: dict[str, Any], weight_accuracy: float = 0.6, weight_match_size: float = 0.4
) -> float:
    """
    ...
    fragment_quality = (
        norm_fragment_size * 0.5 + norm_fragment_count * 0.3 + norm_total_matched * 0.2
    )
...
class SuccessiveHalvingTuner:
    ...
    def sample_file_subsets(self) -> list[list[Path]]:
        ...
        if len(self.file_pool) < 50:
            print(
                f"Warning: Need at least 50 files for non-overlapping subsets. Have {len(self.file_pool)}. Will reuse some files."
            )
    ...
    def evaluate_configuration(
        self, params: dict[str, Any], dataset_files: list[Path]
    ) -> dict[str, Any]:
        ...
        results = run_accuracy_test_with_files(dataset_files, clone_pairs, threshold=self.threshold)

        score = calculate_smart_score(
            results, weight_accuracy=self.weight_accuracy, weight_match_size=self.weight_match_size
        )
    ...
    def run(self, initial_configs: int = 200, round_factors: list[float] = None):
        """
        ...
        """
        if round_factors is None:
            round_factors = [0.25, 0.5, 0.5, 1.0]
...
def main():
    ...
    parser.add_argument(
        "--n-files", type=int, default=100, help="Number of files in pool (default: 100)"
    )
    parser.add_argument(
        "--n-clones", type=int, default=1, help="Number of clones per type per file (default: 1)"
    )
    parser.add_argument(
        "--initial-configs",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="fine_tune_smart_results.json",
    ...
    parser.add_argument("--keep-dataset", action="store_true", help="Keep generated dataset")
