...
def run_accuracy_test_with_dataset(
    ...
    for type_num in types:
        ...
        if type_results["total"] > 0:
            ...
            type_results["avg_match_size"] = (
                type_results["total_match_size"] / type_results["total"]
            )
        else:
...
def print_parameter_set(params: dict[str, Any], score: float, results: dict[str, Any]):
    ...
    for type_name, data in sorted(results["by_type"].items()):
        if data["total"] == 0:
            continue
        print(
            f"  {type_name}: avg_sim={data['avg_similarity']:.1f}%, "
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune plagiarism detection parameters")
    parser.add_argument(
        "--n-files", type=int, default=10, help="Number of files to test per run (default: 10)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--end", type=int, default=50, help="Ending file index (default: 50)")
    parser.add_argument(
        "--types",
        type=int,
        nargs="+",
    ...
    parser.add_argument(
        "--resume", type=str, default=None, help="Resume from previous results file"
    )
    parser.add_argument(
        "--weight-accuracy",
        type=float,
    ...
    if previous_results:
        ...
        combinations = [
            c for c in all_combinations if tuple(sorted(c.items())) not in tested_params
        ]
    ...
    print(
        f"Dataset generated with {args.n_files} files x {len(args.types)} types x {args.n_clones} clones\n"
    )
    ...
    for idx, entry in enumerate(all_results[:5], 1):
        ...
        print(
            f"Performance: detection={summary['overall_detection_rate']:.1f}%, "
