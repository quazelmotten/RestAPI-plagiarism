...
def main():
    ...
    print(f"\n{'=' * 70}")
    print("REALISTIC DB INSERT BENCHMARK")
    print(f"{'=' * 70}")
    print(f"  Pairs:      {args.pairs}")
    print(f"  Runs:       {args.runs}")
    print("  Rows/batch: 100 (current) vs 5000 (improved)")
    print(
        f"  DB calls:   ~{(args.pairs // 100) * 2} (current) vs ~{args.pairs // 5000 + 12} (improved)"
    )
    print()
    ...
    for run in range(args.runs):
        ...
        print(
            f"  Run {run + 1}: {result['results_per_sec']:.0f} results/sec ({result['total_time_s']:.3f}s)"
        )
