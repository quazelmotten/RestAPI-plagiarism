    ...
    for i in range(n):
        ...
        for j in range(n_funcs):
            ...
            for k in range(body_lines):
                ...
                lines.append(
                    f"{indent}result_{k} = {params.split(', ')[0] if params else '0'} + {k}\n"
                )
...
def run_benchmark(args) -> dict:
    ...
    print(f"\n{'=' * 70}")
    print("WORKER PIPELINE BENCHMARK")
    print(f"{'=' * 70}")
    print(f"  Mode:     {mode}")
    ...
    try:
        if mode == "light":
            ...
            print("  Infra:    in-memory (no Redis/PG)")
        else:
            # Full mode: use real Redis + PostgreSQL
            from worker.dependencies import get_cache, get_index, get_redis_client, get_repository
            ...
            print("  Infra:    Redis + PostgreSQL (live)")
        ...
        if use_dataset:
            ...
            if len(file_paths) < n_files:
                print(
                    f"  Warning: only {len(file_paths)} dataset files available (requested {n_files})"
                )
        ...
        print(
            f"  Sizes:    {min(os.path.getsize(f['file_path']) for f in files)}-"
        ...
        idx_stats["files_per_sec"] = (
            len(files) / timings["indexing"] if timings["indexing"] > 0 else 0
        )
        results["indexing"] = idx_stats
        print(
            f"    {idx_stats['files_per_sec']:.1f} files/sec  "
        ...
        cand_stats["pairs_per_sec"] = (
            total_pairs / timings["candidate_finding"] if timings["candidate_finding"] > 0 else 0
        )
        ...
        if len(files) >= 4:
            ...
            print(
                f"    {cross_stats['cross_pairs_found']} cross pairs  "
        ...
        if total_pairs > 0:
            ...
            svc = __import__(
                "worker.services.candidate_service", fromlist=["CandidateService"]
            ).CandidateService(index)
            ...
            store_stats["results_per_sec"] = (
                len(pairs) / timings["result_storage"] if timings["result_storage"] > 0 else 0
            )
            results["result_storage"] = store_stats
            print(
                f"    {store_stats['results_stored']} results  "
        ...
        results["end_to_end"] = {
            ...
            "candidate_finding_pct": timings.get("candidate_finding", 0) / total_time * 100
            if total_time > 0
            else 0,
            "result_storage_pct": timings.get("result_storage", 0) / total_time * 100
            if total_time > 0
            else 0,
        ...
        print(f"\n  {'=' * 50}")
        print(
            f"  END-TO-END: {total_time:.3f}s  |  {e2e_pairs} pairs  |  "
...
def compare_baselines(before_path: str, after_path: str):
    ...
    if "redis_commands" in b.get("candidate_finding", {}) or "redis_commands" in a.get(
        "candidate_finding", {}
    ):
        row(
            "Redis commands",
