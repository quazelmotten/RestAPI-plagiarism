...
echo "  prof-api    - Profile API for specified duration"
echo "  prof-worker - Profile worker for specified duration"
echo "  prof-top    - Real-time profiling of API or worker"
...
cmd_prof_api() {
    DURATION=${2:-30}
    ./scripts/profile-api.sh "$DURATION"
}

cmd_prof_worker() {
    DURATION=${2:-30}
    ./scripts/profile-worker.sh "$DURATION"
}

cmd_prof_top() {
    SERVICE=${2:-api}
    ./scripts/profile-top.sh "$SERVICE"
}
